...
def write():
    ...
    st.markdown("<h1 style='text-align: center; color: white;'>Apps & Websites</h1>", unsafe_allow_html=True)
    ...
    keys = {
    'Apps & Websites': './data/apps_and_websites/apps_and_websites.csv',
    'Kwame Technologists Apps': './data/apps_and_websites/your_apps.csv',   
    } 
    st.header("Dataset")
    ...
    if pick == 'Apps & Websites':
        df = pd.read_csv(keys[pick], encoding='utf8')
        df['Date'] = pd.to_datetime(df['installed_apps/added_timestamp'],unit='s')
        df['App Name'] = df['installed_apps/name']
        df = df[['App Name', 'Date']]
    #df.columns = ['State','Year','Month','Day','Victim','County','Race','Sex','Mob','Offense','Note','2nd Name','3rd Name','Comments','Source']
        ...
        df = pd.read_csv(keys[pick], encoding='utf8')
        df['Date'] = pd.to_datetime(df['admined_apps/added_timestamp'],unit='s')
        df['App Name'] = df['admined_apps/name']
        df = df[['App Name', 'Date']]
    #df.columns = ['State','Year','Month','Day','Victim','County','Race','Sex','Mob','Offense','Note','2nd Name','3rd Name','Comments','Source']
        st.dataframe(df)
