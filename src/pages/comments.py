...
def write():
    ...
    st.markdown("<h1 style='text-align: center; color: white;'>Comments</h1>", unsafe_allow_html=True)
    ...
    keys = {
    'Comments': './data/comments/comments.csv',   
    } 
    st.header("Dataset")
    ...
    df['Date'] = pd.to_datetime(df['data/0/comment/timestamp'],unit='s')
    df['Comment'] = df['data/0/comment/comment']
    df['Title'] = df['title']
    df = df[['Date', 'Comment',  'Title']]
