...
def write():
    ...
    st.markdown("<h1 style='text-align: center; color: white;'>Facebook Gaming</h1>", unsafe_allow_html=True)
    ...
    keys = {
    'Facebook Gaming': './data/facebook_gaming/games.csv',    
    } 
    st.header("Dataset")
    ...
    df['Date'] = pd.to_datetime(df['added_timestamp'],unit='s')
    df['Game'] = df['name']
    df = df[['Game', 'Date']]
    st.dataframe(df)
