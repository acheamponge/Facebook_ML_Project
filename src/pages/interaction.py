...
def write():
    ...
    keys = {
    'Menu Items': './data/interactions/menu_items.csv',
    'People': './data/interactions/people.csv',
      } 
    st.header("Dataset")
    ...
    if pick == 'Menu Items':
        ...
        df['Link'] = df['data/uri']
        df['Number of Clicks'] = df['data/num_clicks']
        df = df[['Name', 'Date', 'Link', 'Number of Clicks']]
        st.dataframe(df)
