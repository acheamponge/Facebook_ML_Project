...
def write():
    ...
    st.markdown("<h1 style='text-align: center; color: white;'>Posts</h1>", unsafe_allow_html=True)
    ...
    df['Date'] = pd.to_datetime(df['attachments/0/data/0/media/creation_timestamp'],unit='s')
    df['Attachement'] = df['attachments/0/data/0/media/uri']
    df['Post'] = df['data/0/post']
    df = df[['Post',  'Date', 'Attachement']]
